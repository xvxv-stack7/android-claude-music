...
> 📌 **先说清楚这一步是平台绑定的**：下面装的是**酷狗**那套接口服务。
> 你的音乐 App 不是酷狗（网易云 / QQ 音乐 / …），就得先找**对应的**那一套本地接口服务。
>
> **找不到也能用**，只是"一起听"只剩一半：它照样知道你听什么、也能给你放歌
> （这两件事走系统媒体接口，跟平台无关），但**跟不上你唱到哪一句** —— 歌词那层没了。
>
> 本项目默认的这套：**[KuGouMusicApi](https://github.com/MakcRe/KuGouMusicApi)**（第三方逆向项目，仅供学习交流）。
...
```
♪ 歌词: 《桃子》唱到「[0:19] 不然怎么会挑中我呢」 [规则: 有意思就接一句聊，没感觉就放着不吭声]
```
...
它自己去副屏上走一圈：**归位** → 点顶部`搜索` → 灌歌名 → **点结果行** —— 歌就放起来了。

**两个设计，都是踩出来的**：

- **归位不能省**：一首放完，App 可能停在歌手页、播放页、随便哪儿 ——
  不先退回主界面就点"搜索"，点出来的是别的东西。归位用返回键循环，**不 force-stop**，歌不断。
- **结果行的坐标是现读的**：脚本从控件树里找**含歌名的那一行**再点，**不写死坐标** ——
  换台手机、换个搜索词都能用。整个流程里写死的只有"顶部搜索入口"一个点（`SEARCH_X/Y`）。

> ⚠️ **别走"歌单页右上角搜索"那条**：那条会让输入法弹出**全屏提取框**盖住整个上半屏
> （`0,0~1124,270`），结果行**点不动**（点哪儿都被输入法吃掉），得先收键盘 ——
> 而输入法的"完成"键每个输入法位置都不一样。**顶部搜索这条没这问题。**
