...
流程：`我的` → `我喜欢` → 右上`搜索` → 灌歌名 → **收键盘** → 点结果第一行的歌名区。

**"收键盘"那步是关键**：输入法的提取框盖着整个上半屏（`0,0~1124,270`），
**不收就点不动结果行** —— 点哪儿都被输入法吃掉。这一步卡了很久才找到。

**歌名从 `list` 拿**（纯 API，一次全出），别在副屏上一个个读树。

⚠️ **所有坐标都得你自己量**（脚本顶部那五个变量）：每台手机、每个 App 都不一样。
用 `vd.sh tree` 读控件坐标、`vd.sh shot` 截图看着量。**别抄别人的**。
